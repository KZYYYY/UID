    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (_, img, vid, target_cam) in enumerate(train_loader_stage2):
            ...
            with amp.autocast(enabled=True):
                ...
                # score, image_features = model(x=img, text_feats=text_feats, label=target, epoch=epoch)
                ...
                logits_view3 = image_features[2] @ text_features[:, 2].t()

                logits = [(logits_view1 + logits_view2 + logits_view3)/3, logits_moe]
            ...
            acc = (acc1 + acc2) / 2
            # acc = (logits[0].max(1)[1] == target).float().mean()
