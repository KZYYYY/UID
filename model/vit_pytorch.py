...
class HybridEmbed(nn.Module):
    ...
    def __init__(self, backbone, img_size=224, feature_size=None, in_chans=3, embed_dim=768):
        ...
        if feature_size is None:
            ...
            feature_size = feature_size if isinstance(feature_size, tuple) else (feature_size, feature_size)
...
class PatchEmbed_overlap(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        img_size = img_size if isinstance(img_size, tuple) else (img_size, img_size)
        patch_size = patch_size if isinstance(patch_size, tuple) else (patch_size, patch_size)
        stride_size_tuple = stride_size if isinstance(stride_size, tuple) else (stride_size, stride_size)
...
def vit_base_patch16_224_TransReID(img_size=(256, 256), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
