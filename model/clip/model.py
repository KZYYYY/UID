...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND, gradcam可视化过程中该处需注释
        ...
        # x_out = x_out.transpose(1, 2).reshape((N, C, patch_size, patch_size))
        # x_up = F.interpolate(x_out,
        #                      size=(patch_size * 2, patch_size * 2),
        #                      mode='bilinear')
        # x_down = F.interpolate(x_out,
        #                        size=(patch_size // 2, patch_size // 2),
        #                        mode='bilinear')
        # x_up = self.up_scale(x_up)
        # x_out = self.keep_scale(x_out)
