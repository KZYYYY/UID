...
from torch.nn.functional import avg_pool2d, softmax
...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg):
        ...
        # DA_MV
        self.max_sent_num = 3  # expert_num
    ...
    def forward(self, x=None, label=None, img_feats=None, text_feats=None, cam_label=None, get_image=False,
        ...
        if self.training:
            ...
            fq_list.append(MV1_feat.half())

            MV2_feat = self.insEn_2(img_feature_proj)
            MV2_feat = self.ln_ins(MV2_feat).half()
            fq_list.append(MV2_feat.half())

            MV3_feat = self.insEn_3(img_feature_proj)
            MV3_feat = self.ln_ins(MV3_feat).half()
            fq_list.append(MV3_feat.half())
            ...
            # return [cls_score, cls_score_proj], [MV1_feat], [Moe_reid_pred, moe_consistency_loss]
            return [cls_score, cls_score_proj], [MV1_feat, MV2_feat, MV3_feat], [Moe_reid_pred, moe_consistency_loss]
        else:
